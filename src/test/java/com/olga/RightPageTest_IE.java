 ...
 */
public class RightPageTest_IE extends TestBasis {
    @Before
    public void start() {
        if (driver == null) {
    ...
    public void checkProductItem() {
        ...
        if (!mainPage.findElement(By.cssSelector("s.regular-price")).getCssValue("text-decoration").equals("line-through")) {
        ...
        if (!productPage.findElement(By.cssSelector("s.regular-price")).getCssValue("text-decoration").equals("line-through")) {
    ...
    public boolean isColorGrey(WebElement element, By locator) {
        ...
        String[] colorArr = color.substring(5, color.length() - 1).split(", ");
